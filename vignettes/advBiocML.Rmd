...
does not have the BCR/ABL fusion mutation.
Forty (40) patients contributed data to these displays. 
...
1. Which plot exhibits the least misclassification of training samples?  a b c d
1. Which statement is true?  Based on the training data: 
...
1. True or false: All procedures agree that a patient presenting with DDR1 = 7.7 and
...
4. We can use KEGGREST to obtain information related to ALL.
```
...
following extract and display of ALL-relevant gene expression
patterns.
...
The process that leads to coloring the plane in this example is
an illustration of **generalization** in machine learning.  Specifically,
even though our data consist of 40 pairs of measurements on DDR1 and DYRK4,
...
the teal-colored region at the top center of the rpart panel.  We start at the top
...
The resampling process reduces the risk of tuning the procedure too
...
red from blue points.  But transformation from $(x,y)$ to $(x,y,x^2+y^2)$
are pushed to larger values in the third dimension.  A separating hyperplane
can then be found using linear discriminant analysis.
